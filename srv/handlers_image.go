...
// HandleAPIParseImage parses transaction image using Claude Vision API
func (s *Server) HandleAPIParseImage(w http.ResponseWriter, r *http.Request) {
	...
	if apiKey == "" {
		s.writeJSON(w, ParseImageResponse{Error: "이미지 분석 API 키가 설정되지 않았습니다. 환경변수 ANTHROPIC_API_KEY를 설정해주세요."})
		return
	}

	// Call Claude Vision API
	transactions, err := parseImageWithClaude(apiKey, imageData, mediaType)
	if err != nil {
		...
		"messages": []map[string]interface{}{
			{
				...
				"content": []map[string]interface{}{
					...
					{
						"type": "text",
						"text": `이 이미지는 신용카드 이용내역 목록입니다. 각 거래를 JSON 배열로 추출해주세요.
